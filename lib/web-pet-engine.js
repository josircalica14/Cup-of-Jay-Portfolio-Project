...
function isHovered(state, input, behavior) {
  ...
  const centerY = input.boundsHeight - input.sprite.height / 2;
...
export function stepPet(state, input, behavior, random = Math.random) {
  ...
  const petY = boundsHeight - input.sprite.height / 2;
  const isMouseNear = input.mouse && Math.hypot(input.mouse.x - next.x, input.mouse.y - petY) < behavior.followDist;
  const isWall = input.mouse && input.mouse.y < behavior.wallZone;

  if (isHovered(state, input, behavior)) {
    next.mode = "hover";
